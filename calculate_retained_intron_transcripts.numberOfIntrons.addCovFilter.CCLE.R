...
if(TRUE) {
  ...
  use.ir.geneninfo.grch37.flag = 1
  use.ir.geneninfo.grch38.flag = 0
...
path = '/group/bioinformatics/Projects/CRI-BIO-589-RO-SPitroda-rbao/results/integrate'
setwd(path)
...
if(TRUE) {
  ...
  ir.path = 'Z:/Projects/CRI-BIO-589-RO-SPitroda-rbao/results/integrate/'
  ir.file = 'CCLE.irfinder.clean_keepNonUniformIntronCoverOrLowCovDP6.tsv' ## grch37
...
if(TRUE) {
  ...
  ir = read.delim(file.path(ir.path, ir.file),stringsAsFactors = F)
  transxinfo = read.delim(transxinfo.file, stringsAsFactors = F)
  # subtype = read.delim(subtype.file, stringsAsFactors = F)
  ir.geneinfo = read.delim(ir.geneinfo.file, stringsAsFactors = F)
...
if(TRUE) {
  ...
  ir.coding.stats = cbind(
    data.frame(table(ir.coding$Sample[ir.coding$IRratio  >= 0.00])),
    data.frame(table(ir.coding$Sample[ir.coding$IRratio  >= 0.05])),
    data.frame(table(ir.coding$Sample[ir.coding$IRratio  >= 0.10])),
    data.frame(table(ir.coding$Sample[ir.coding$IRratio  >= 0.15])),
    data.frame(table(ir.coding$Sample[ir.coding$IRratio  >= 0.20])))
