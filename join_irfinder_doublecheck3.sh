...
projPath=/gpfs/data/pitroda-lab/RZ73/irfinder
...
samples=`ls -al ${projPath}/${prefix}*/IRFinder-IR-${irtype}.txt | sed 's/\//\t/g' | cut -f 7 | sort `
...
if [ $run -eq 4 ]; then 
	...
	awk -F"\t" 'NR==1 || (NR>1 && $5 ~/clean/ && $10 + $20 >= 6 && $22 ~/-|LowCov/)' ${project}.rnaseq.irfinder.tsv > ${project}.irfinder.clean_LowCovDP6.tsv
