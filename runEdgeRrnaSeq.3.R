...
#    RPKM <- rpkm(subdata[foo,],gene.length=geneData[foo,]$width)
    logCPM <- cpm(subdata, log=TRUE)[foo,]
    stopifnot(rownames(lrt$table)==rownames(logCPM))
...
#    print(topTags(lrt))
    ...
    df <- cbind(as.data.frame(geneData[foo,]),logCPM,lrt$table,adj.p=p.adjust(lrt$table$PValue,method="BH"))
