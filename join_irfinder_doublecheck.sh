...
project=CCLE
prefix=SRR
irtype=nondir ## nondir for unstranded rnaseq lib; dir for stranded rnaseq lib; irfinder automaticaly detects lib type; if rnaseq is stranded, it will also output dir output file, in addition to the default nondir output file by default; so for unstranded lib, you will only see nondir output file, but for stranded lib, you will see both nondir and dir output file, and you should use the dir output file for further analysis; note the automate detection of lib type may not be accurate, so sometimes you will see some samples have both nondir and dir output file, some only have nondir output file ... in this case, use nondir output file which is available for *all samples* for further analysis! Otherwise, if dir output file is available for *all samples*, use dir output file.

## ------------------------------------------------

projPath=/group/bioinformatics/Projects/CRI-BIO-589-RO-SPitroda-rbao/results/rnaseq/${project}_multisample
...
if [ $run -eq 1 ]; then 
	...
	for file in `ls -d ../../${project}_samples/${prefix}*/intron_retention/irfinder/${prefix}*/`; do echo $file; ln -s $file;done
...
samples=`ls -al ../${project}_samples/${prefix}*/intron_retention/irfinder/${prefix}*/IRFinder-IR-${irtype}.txt | awk '{print $9}' | sed 's/\//\t/g' | cut -f 3 | sort `
...
if [ $run -eq 2 ]; then 
	...
	cat irfinder.header.0 > $out 

	for sample in $samples; do 
		in=""
		in="../${project}_samples/$sample/intron_retention/irfinder/$sample/IRFinder-IR-${irtype}.txt"
		echo $sample 
		echo $in
