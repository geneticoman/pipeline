...
path = '/gpfs/data/pitroda-lab/ncbi/GSE100412/IRFinder_out'
setwd(path)
...
## data files 
if(TRUE) {
  transxinfo.file = 'gencode.vM22.annotation.gtf.transcriptinfo'
  ...
  ir.path = path
  ir.file = 'GSE100412.irfinder.clean_LowCovDP6.tsv' ## grch37
  ...
  if(use.ir.geneninfo.grch37.flag == 1) {
    ...
    ir.geneinfo.file = 'gencode.vM22.annotation.gtf.geneinfo'
  }
...
if(TRUE) {
  ...
  transxinfo = read.delim(file.path("/gpfs/data/pitroda-lab/ReferenceData/anno/", transxinfo.file), stringsAsFactors = F)
  # subtype = read.delim(subtype.file, stringsAsFactors = F)
  ir.geneinfo = read.delim(file.path("/gpfs/data/pitroda-lab/ReferenceData/anno/", ir.geneinfo.file), stringsAsFactors = F)
